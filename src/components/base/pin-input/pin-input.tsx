...
const Group = ({ inputClassName, containerClassName, width, maxLength = 4, ...props }: GroupProps) => {
    ...
    return (
        <OTPInput
            ...
            containerClassName={cx("flex flex-row gap-3 rtl:flex-row-reverse", size === "sm" && "gap-2", heights[size], containerClassName)}
...
const sizes = {
    xs: "size-12 px-2 py-0 text-display-sm font-medium",
    ...
    return (
        <div
            className={cx(
                ...
                size === "lg" ? "text-display-xl font-medium" : "text-display-md font-medium",
            )}
        />
    );
};
