            ...
            <div className="mx-auto max-w-container px-4 md:px-8">
                ...
                <div className="grid grid-cols-2 justify-center gap-2 sm:grid-cols-[repeat(auto-fit,220px)] md:gap-4">
                    {tests.map((test, index) => (
                        <div
                            key={index}
                            className={`relative flex h-full flex-col gap-6 overflow-hidden rounded-2xl border p-6 shadow-md transition-all duration-200 hover:-translate-y-1 hover:shadow-xl sm:gap-y-12 sm:py-12 md:w-[220px] ${test.borderColor} ${test.bgColor}`}
                        >
                            <div
                                className="absolute top-0 right-0 size-24 rounded-bl-full bg-linear-to-br from-primary-solid/20 to-white/10"
                                aria-hidden="true"
                            />
                            {/* Test Logo */}
