...
export const ModalOverlay = (props: AriaModalOverlayProps) => {
    return (
        <AriaModalOverlay
            ...
            className={(state) =>
                cx(
                    "fixed inset-0 z-50 flex min-h-dvh w-full items-center justify-center overflow-y-auto bg-overlay/70 px-4 pt-4 pb-[clamp(16px,8vh,64px)] outline-hidden backdrop-blur-[6px] sm:items-center sm:justify-center sm:p-8",
...
export const Modal = (props: AriaModalOverlayProps) => (
    <AriaModal
        ...
        className={(state) =>
            cx(
                "max-h-full align-middle outline-hidden max-sm:overflow-y-auto max-sm:rounded-xl",
